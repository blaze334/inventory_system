...
if(isset($_POST['submit'])){
    ...
    $faculty_id = validate(@$_POST['faculty_id']);
    ...
    if(!($pass_ext == "jpg" || $pass_ext == "png" || $pass_ext == "jpeg" || $pass_ext == "gif")){
        $errors[] = "Passport must be in either jpg, png, jpeg or gif format";
    }    
    if(!preg_match("/^[A,N][0-9]+$/", $staff_no)){
        $errors[] = "Staff Number is required and should COntain A for Academic staff and N for Non Academic Staff";
    }
	if($faculty_id == ""){
        $errors[] = "Please Select Faculty";
    }
if($rank == ""){
        $errors[] = "Please Select Rank";
    }
    ...
    if(count($errors) == 0){
        $select = mysqli_query($conn, "SELECT id FROM staff WHERE staff_no='$staff_no'");
        if (mysqli_num_rows($select) > 0) {
            ...
            //END OF PASSPORT
            $insert = mysqli_query($conn, "INSERT INTO `staff`(`first_name`, `other_names`, `gender`, `dob`, `phone`, `email`, `state`, `lga`, `address`, `passport`, `staff_no`, `faculty_id`, `rank`, `dept_id`) VALUES ('$first_name','$other_names','$gender','$dob','$phone','$email','$state','$lga','$address','$passport','$staff_no','$faculty_id','$rank','$dept_id')");
            $student_id = mysqli_insert_id($conn);
            $insert2 = mysqli_query($conn, "INSERT INTO `user`(`username`,`password`,`user_cat`) VALUES ('$staff_no',MD5('$pass'),'staff')");
            ...
            <form method="post" enctype="multipart/form-data">
                        ...
                        <div class="col-sm-8">
                            <div style="width: 100px;height: 150px;">
                                <div style="width:100px; height:125px;border:2px solid #555; text-align: center;">
                                    <img src="#" style="width: 100%;height: 100%;" id="preview" alt="Passport"/>
                                </div>
                                <input type="file" name="passport" id="passport" class="inputPassport" required/>
                ...
                <div class="form-row">          
                    ...
                    <div class="col-sm-6">
                        ...
                        <div class="col-sm-8">
                            <input name="staff_no" value="<?=@$_POST['adm_no']?>" required type="text" class="form-control" placeholder="Staff Number" maxlength="20"/>
                        </div>
                    </div>
                    ...
                    <div class="col-sm-6">
                        ...
                        <div class="col-sm-8">
                            <select class="form-control" name="faculty_id" required>
                                ...
                                <?php
                                    $faculty_id = array("NULL","Science","Social Science","Art & Social Science");
                                    foreach($faculty_id as $value){
                                        echo "<option value='".$value."' ";
                                        if(@$_POST['faculty_id'] == $value)echo 'selected="selected" ';
                                        echo ">".$value."</option>";
                                    }
                                ?>
                            </select>
                    ...
                    <div class="col-sm-6">
                        ...
                        <div class="col-sm-8" >
                            <select class="form-control" name="rank" required>
                                <option value="" disabled selected>Get your Rank</option>
                                <?php
                                    $rank = array("","Registrar","HOD Computer","Dean","Cashier");
                                    foreach($rank as $value){
                                        echo "<option value='".$value."' ";
                                        if(@$_POST['rank'] == $value)echo 'selected="selected" ';
                                        echo ">".$value."</option>";
                                    }
                                ?>
                            </select>
                ...
                <div class="alert alert-info msg"><span style="color: red;">NB:</span> Your Staff Number is your USERNAME</div>
                <div class="form-row">          
                    <div class="col-sm-6">
                        <label class="col-sm-4 control-label">Password:</label>
                        <div class="col-sm-8">
                            <input name="pass" value="<?=@$_POST['pass']?>" required type="password" class="form-control" placeholder="Password" maxlength="20"/>
                        </div>
                    </div>
                    <div class="col-sm-6">
                        <label class="col-sm-4 control-label">Confirm Password:</label>
                        <div class="col-sm-8">
                            <input name="cpass" value="<?=@$_POST['cpass']?>" required type="password" class="form-control" placeholder="Confirm Password" maxlength="20"/>
                        </div>
                    </div>
                </div>
                <br><br>
                <div class="col-sm-12">
                    <div class="col-sm-2 pull-right">
                        <button type="submit" name="submit" class="btn btn-submit col-sm-12"> Register</button>
                    </div>
                </div>
            </form> 
        </div>
