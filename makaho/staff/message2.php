...
if(isset($_GET['staff_no']) && !isset($_GET['download'])){
    ...
    //UPDATE VIEW
    $view = "%#".$username."#%";
    $sel1 = mysqli_query($conn, "SELECT * FROM message WHERE reciever = '$course_code' AND view NOT LIKE '$view'") or die(mysqli_error($conn)); 
...
if(isset($_POST['send-text']) && str_replace(" ", "", $_POST['text']) != ""){
    ...
    $content = $_POST['text'];
    $sender = $username;
    $reciever = $course_code;
    $type = "text";
    $time = time() - 86400;

    $view = "#".$username."#";
    $insert = mysqli_query($conn, "INSERT INTO message(content, sender, reciever, type, view, transmission, `time`) VALUES('$content', '$sender', '$reciever', '$type', '$view', 'broadcast', '$time')");
    if($insert){ 
...
if(isset($_POST['send-file']) && str_replace(" ", "", $_FILES["file"]["name"]) != ""){
    ...
    if($ext!="mp4" && $ext!="mp3" && $ext!="pdf" && $ext!="docx" && $ext!="doc" && $ext!="txt" && $ext!="jpg" && $ext!="png" && $ext!="jpeg" && $ext!="gif"){
        ...
        $view = "#".$username."#";
        $insert = mysqli_query($conn, "INSERT INTO message(content, sender, reciever, type, view, transmission, `time`) VALUES('$file', '$sender', '$reciever', '$type', '$view', 'broadcast', '$time')");
        if($insert){ 
            echo "<script>alert('Message Sent')</script>";   
        }
    }
...
<div>
    <section style="background-color:#DDD; text-align:center; padding: 20px 0px;overflow: hidden;"><br><br>
        <div class="panel col-sm-offset-2 col-sm-8" style="background-color:#FFF;"><br/>
            <div class="panel-heading bcolor-blue" style="color:#fff !important;"><i class="glyphicon glyphicon-comment"></i><b> <?=$course_code?> Forum</b></div>
            <div class="panel-body">
                <div class="well col-sm-12" style="padding-top: 8px; overflow: hidden;">
                    <?php
                        $select = mysqli_query($conn, "SELECT * FROM message WHERE reciever='$course_code'") or die(mysqli_error($conn));        
                        ...
                        if($num == 0){
                            ...
                            $colors = array("blue","red","green-f","maroon","yellow","navy","olive","orangered","purple","seagreen","brown");
                            while($fetch = mysqli_fetch_array($select)) {
                                ...
                                $sel1 = mysqli_query($conn, "SELECT * FROM staff WHERE id='$username'") or die(mysqli_error($conn));        
                                $num1 = mysqli_num_rows($sel1);
                                ...
                                if($sender != $username){
                                    $name = $sender;
                                    // CHECK FOR LECTURER
                                    $sel2 = mysqli_query($conn, "SELECT * FROM staff WHERE id='$sender'") or die(mysqli_error($conn));        
                                    $num2 = mysqli_num_rows($sel1);
                                    if($num2 > 0){
                                        $name .= "(student)";
                                        $id = 10;
                                    }
                                    $class = "left-arrow ".$colors[$id];                                        
                                }
                                else{
                                    $class = "right-arrow col-sm-offset-3 ".$colors[$id];
                                    $name = "Me";
                                }
